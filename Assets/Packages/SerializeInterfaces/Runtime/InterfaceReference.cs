	...
	public class InterfaceReference<TInterface, UObject> where UObject : Object where TInterface : class
	{
		...
		public TInterface Value
		{
			get
			{
				...
				// Check if type is compitable with TInterface (as operator returns null in that case)
				// Not throwing exception in order to not block the user interactions
				var @interface = _underlyingValue as TInterface;
				if (@interface == null)
					Debug.LogError($"{_underlyingValue} needs to implement interface {nameof(TInterface)}.");
			...
			set
			{
				...
				else
				{
					...
					if (newValue == null)
						Debug.LogError($"{value} needs to be of type {typeof(UObject)}.");
